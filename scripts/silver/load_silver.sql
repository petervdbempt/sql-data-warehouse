...
Description: Stored procedure to load transformed and cleansed data into the
             'silver' schema from the 'bronze' schema.

WARNING:
This stored procedure TRUNCATES existing data in all silver tables before loading.
Executing this procedure will permanently overwrite current silver-layer records.
===============================================================================
*/
...
BEGIN
    ...
    BEGIN TRY
        ...
        WHERE flag_last = 1; -- Keep the most recent record for each customer
        ...
        SELECT
            ...
            prd_nm,
            ISNULL(prd_cost, 0) AS prd_cost, -- Replace missing product costs with zero
            CASE 
            ...
            CAST(
                ...
                AS DATE
            ) AS prd_end_dt -- Set end date to one day before the next start date
        ...
        SELECT 
            ...
            CASE 
                ...
                ELSE sls_sales
            END AS sls_sales, -- Recalculate sales when the source value is missing or invalid
            sls_quantity,
            CASE 
                WHEN sls_price IS NULL OR sls_price <= 0 
                    THEN sls_sales / NULLIF(sls_quantity, 0)
                ELSE sls_price
            END AS sls_price -- Derive price when the source value is missing or invalid
        ...
        SELECT
            ...
            CASE
                ...
                ELSE bdate
            END AS bdate, -- Replace future birthdates with NULL
            CASE
